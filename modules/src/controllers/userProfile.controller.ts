...
export class UserProfileController {
    ...
    static async updateUserDetails(request:Request, h:ResponseToolkit) {
        try {
            ...
            const {interestedIn, areaLocality, hobbies, mobileNo, dateOfBirth, gender} = request.payload as {
                interestedIn: string;
                areaLocality: string;
                hobbies: string[];
                mobileNo: string;
            ...
            const updatedUser = await User.update({interestedIn, areaLocality, hobbies, mobileNo, dateOfBirth, gender}, { where: {id:isUser.id}});
